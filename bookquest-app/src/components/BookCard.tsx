...
export default function BookCard({ book, isFavorite, onToggleFavorite, onViewDetails }: BookCardProps) {
  ...
  // ✅ Fixed: Fallback image URLs for when Amazon blocks access
  const getAlternativeImageUrl = (originalUrl: string, isbn: string | null = null): string[] => {
    const fallbackUrls = []
    
    // Try to extract ISBN from Amazon URL
    const amazonMatch = originalUrl.match(/\/images\/P\/(\d+)/)
    const extractedIsbn = amazonMatch ? amazonMatch[1] : isbn
    
    if (extractedIsbn) {
      // Alternative book cover APIs
      fallbackUrls.push(
        `https://covers.openlibrary.org/b/isbn/${extractedIsbn}-L.jpg`,
        `https://books.google.com/books/content/images/frontcover/${extractedIsbn}?fife=w300-h400`,
        `https://images.isbndb.com/covers/${extractedIsbn.slice(0,2)}/${extractedIsbn.slice(2,4)}/${extractedIsbn.slice(4,6)}/${extractedIsbn}.jpg`
      )
    }
  ...
  // ✅ Handle image errors with fallback attempts
  const handleImageError = () => {
    console.log('Image failed to load:', imageSrc)
    
    if (!imageError) {
      const fallbackUrls = getAlternativeImageUrl(book.image)
      
      // Try first fallback
      if (fallbackUrls.length > 0 && imageSrc === book.image) {
        console.log('Trying fallback:', fallbackUrls[0])
        setImageSrc(fallbackUrls[0])
        return
      }
      
      // Try second fallback
      if (fallbackUrls.length > 1 && imageSrc === fallbackUrls[0]) {
        console.log('Trying second fallback:', fallbackUrls[1])
        setImageSrc(fallbackUrls[1])
        return
      }
    }
        ...
        <div className="relative w-full h-52 mb-4 overflow-hidden rounded-lg shadow-md">
          {imageSrc && !imageError && (
            <>
              ...
              {/* ✅ Fixed: Better error handling and fallback system */}
              <Image
                src={imageSrc} 
                alt={book.title}
                width={300}
            ...
            <div className="w-full h-full bg-gradient-to-br from-slate-200 via-slate-300 to-slate-400 dark:from-slate-700 dark:via-slate-600 dark:to-slate-500 rounded-lg flex items-center justify-center">
              <div className="text-center text-slate-500 dark:text-slate-400">
                ...
                {process.env.NODE_ENV === 'development' && (
                  <div className="text-xs mt-1 opacity-75">403 Error</div>
                )}
              </div>
            </div>
          )}
